...
public class UserServiceImpl implements UserService {
	...
	public String registerUser(UserDto userDto, BindingResult result, HttpSession session) {
		...
		if (result.hasErrors())
			...
			User user = new User(null, userDto.getName(), userDto.getEmail(), encoder.encode(userDto.getPassword()),
					userDto.getMobile(), userDto.getGender(), otp, false, "ROLE_USER", LocalDateTime.now());
			userDao.save(user);
	...
	@Override
	public String buyNow(Long id, HttpSession session, Principal principal) {
		Product product = productRepository.findById(id).orElseThrow();
		if (product.getStock() > 0) {
			String email = principal.getName();
			User user = userDao.findByEmail(email).orElseThrow();
			...
			product.setStock(product.getStock() - 1);
			productRepository.save(product);
			session.setAttribute("success", "Product Added to Cart. Proceeding to Checkout.");
